...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  //Make board
  board_t* bo = malloc(1*sizeof(*bo));
  bo->board = malloc(h*sizeof(*(bo->board)));
  for(int i=0; i<h;i++){
    bo->board[i] = malloc(w*sizeof(*(bo->board[i])));
    for(int j=0; j<w;j++){
      bo->board[i][j] = UNKNOWN;
    }  
  }
  bo->width = w;
  bo->height = h;
  bo->totalMines = numMines;
  //add mines
...
int countMines(board_t * b, int x, int y) {
  ...
  int w = (b->width)+2;
  int h = (b->height)+2;
  int** bn = malloc(h*sizeof(*bn));
  for(int i = 0; i<h; i++){
   bn[i] = malloc(w*sizeof(*bn[i]));
  }
  for(int i=1; i<(h-1);i++){
    for(int j =1; j<(w-1);j++){
      bn[i][j] = b->board[i-1][j-1];
    }
  }
  for(int i=0;i<w;i++){
    bn[0][i] = UNKNOWN;
  ...
  if(IS_MINE(bn[y][x+1]) == 1){
    count = count+1;
  }
  if(IS_MINE(bn[y][x+2]) == 1){
    count = count+1;
  }
  if(IS_MINE(bn[y+1][x]) == 1){
    count = count+1;
  }
  if(IS_MINE(bn[y+1][x+2]) == 1){
    count = count+1;
  }
  if(IS_MINE(bn[y+2][x+1]) == 1){
    count = count+1;
  }
  if(IS_MINE(bn[y+2][x]) == 1){
    count = count+1;
  }
  if(IS_MINE(bn[y+2][x+2]) == 1){
    count = count+1;
  } 
